...
contract BankFunctionOracle is Ownable {
  ...
  event InvokeFiatDepositEvent(uint256 amount ,string bankAccountNumber, uint256 id);
    event InvokeFiatWithdrawEvent(uint256 amount ,string bankAccountNumber, uint256 id);
    event SetFiatDepositResultEvent(string _result, address _callerAddress, uint256 _id);
    event SetFiatWithdrawResult(string _result, address _callerAddress, uint256 _id);
