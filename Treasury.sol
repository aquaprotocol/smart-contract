...
contract Treasury is ERC20, ERC20Detailed  {
    ...
    event Mint(address a, uint256 amount);
    event Burn(address a, address b, uint256 amount);
    event BurnFiat(address a, address b, uint256 amount);
        event InternalTransfer(address _target, uint256 _amount);

    constructor () public ERC20Detailed("PLCOIN", "PLC",18) {
    ...
    function internalTransfer(address asset, address _target, uint256  _amount)
    external {
